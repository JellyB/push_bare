...
case "$1" in
    start)
        ./push-server.jar start
    ;;
    stop)
        ./push-server.jar stop
    ;;
    type)
        ./push-server.jar type
    ;;
    restart)
        ./push-server.jar restart
    ;;
    dump)
        do_dump
    ;;
    *)
        echo "Usage ${0} <start|stop|type|restart|dump>"
        exit 1
    ;;
esac
